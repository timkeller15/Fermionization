%% Simulation parameters for N = 2. Everything in dimensionless units as detailed in publication. 
% System
N = 2; % Number of immersed particles
Ngrid = 1024; % Position grid points (power of 2 for split-step algorithm)
NBEC = 1e4; % Number of BEC particles
gBEC = 1; % BEC intra-particle interaction strength
posmax = 5; % Position grid range
LTG = 8; % Impurity box potential range 
wall = 1e8; % Value of the box potential edges
% Time-Evolution
dt = 1e-4; % Time step
cutoff = 1e-5; % Energy convergence criterion 
steps = 5e5; % Number of time steps
